# gemini_frostmart.py — FrostMart UK Predictive Analytics Report Generator
...
# Generate FrostMart UK Analytics Report
def generate_frostmart_report(knowledge_base: str) -> str:
    """Generate a fully formatted FrostMart UK business insights report."""
    if genai is None:
        return "⚠️ google.genai module not installed. Cannot generate report."

    prompt = f"""
You are Francis Afful Gyan, Business Intelligence Specialist for FrostMart UK. 
Generate a comprehensive, professional business insights report in plain text, fully completed with all sections.

REPORT REQUIREMENTS:
- Include: Executive Summary, Financial Performance Analysis, Product & Regional Insights, Sales/Wastage/Pricing Insights, Seasonality & Trends, Model Development & Evaluation, Deployment & Integration, Key Challenges & Opportunities, Strategic Recommendations
- Use only FrostMart UK knowledge base
- Provide exact numbers, percentages, and metrics (£, %, decimals)
- Format cleanly: No hashtags (#), no bold (**), no Markdown symbols
- Each section should be fully written, not truncated
- Include actionable, data-driven recommendations with success metrics
...
if __name__ == "__main__":
    try:
        ...
        report_text = generate_frostmart_report(kb_text)
        print(report_text)
